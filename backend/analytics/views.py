...
def analytics_dashboard(request):
    ...
    # Downloads per month
    downloads_data = (
        DownloadLog.objects.filter(timestamp__gte=start_date)
        .extra(select={"month": "strftime('%%Y-%%m', timestamp)"})
        .values("month")
    ...
    search_data = (
        SearchLog.objects.filter(timestamp__gte=start_date)
        .values("query")
        .annotate(count=Count("id"))
        .order_by("-count")[:10]
    )
    ...
    popular_resources = (
        Resource.objects.filter(download_count__gt=0)
