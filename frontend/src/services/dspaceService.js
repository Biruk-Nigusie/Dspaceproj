import { DSPACE_API_URL } from "./utils/endpoints";
...
class DSpaceService {
	...
	async createWorkspaceItem(collectionId) {
		try {
			const response = await fetch(
				`${DSPACE_API_URL}/submission/workspaceitems`,
				{
					method: "POST",
					headers: {
	...
	async uploadFile(workspaceItemId, file) {
		try {
			...
			const response = await fetch(
				`${DSPACE_API_URL}/submission/workspaceitems/${workspaceItemId}/sections/upload`,
				{
					method: "POST",
					credentials: "include",
					body: formData,
	...
	async getWorkspaceItems() {
		try {
			const response = await fetch(
				`${DSPACE_API_URL}/submission/workspaceitems?size=10`,
				{
					credentials: "include",
					headers: { Accept: "application/json" },
				},
