import { Eye, EyeOff, File, LogIn, Search, Upload } from "lucide-react";
...
const IntegratedUpload = () => {
	...
	const fetchUploadedFiles = async () => {
		try {
			const response = await fetch(
				"http://localhost:8000/api/resources/uploaded-files/",
				{
					credentials: "include",
				},
			);
	...
	const handleIntegratedUpload = async (e) => {
		...
		try {
			...
			const djangoResponse = await fetch(
				"http://localhost:8000/api/resources/upload-file/",
				{
					method: "POST",
					body: djangoFormData,
					credentials: "include",
			...
			if (dspaceAuth && formData.collectionId) {
				try {
					const workspaceItem = await dspaceService.createWorkspaceItem(
	...
	const handleSearch = async () => {
		try {
			const response = await fetch(
				`http://localhost:8000/api/resources/search-files/?q=${encodeURIComponent(
					searchQuery,
				)}`,
				{
					credentials: "include",
						...
						{uploadedFiles.map((file) => (
							...
							>
								...
								<p className="text-xs text-gray-500">
									Django • {new Date(file.created_at).toLocaleDateString()}
								</p>
							</div>
						))}
