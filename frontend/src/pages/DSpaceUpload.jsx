import { Eye, EyeOff, File, LogIn, Upload, X } from "lucide-react";
...
const DSpaceUpload = () => {
	...
	const checkAuthStatus = async () => {
		try {
			const response = await fetch(
				"http://localhost:8080/server/api/authn/status",
				{
					credentials: "include",
					headers: {
	...
	const handleLogin = async (e) => {
		...
		try {
			...
			const body = `password=${encodeURIComponent(
				loginData.password,
			)}&email=${encodeURIComponent(loginData.user)}`;
	...
	const fetchUploadedItems = async () => {
		try {
			const response = await fetch(
				"http://localhost:8080/server/api/submission/workspaceitems?size=10",
				{
					credentials: "include",
					headers: {
						Accept: "application/json",
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const createResponse = await fetch(
				"http://localhost:8080/server/api/submission/workspaceitems",
				{
					method: "POST",
					headers: {
			...
			if (!createResponse.ok) {
				throw new Error(
					`Failed to create workspace item: ${createResponse.status}`,
				);
			}
			...
			if (metadataUpdates.length > 0) {
				const patchResponse = await fetch(
					`http://localhost:8080/server/api/submission/workspaceitems/${workspaceItemId}`,
					{
						method: "PATCH",
						headers: {
							"Content-Type": "application/json",
			...
			if (files.length > 0) {
				for (const file of files) {
					...
					const uploadResponse = await fetch(
						`http://localhost:8080/server/api/submission/workspaceitems/${workspaceItemId}/sections/upload`,
						{
							method: "POST",
							credentials: "include",
							body: fileFormData,
						...
						<div>
							...
							<div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
								...
								<label htmlFor="file-upload" className="cursor-pointer">
									<span className="inline-block bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300">
										Select Files
									</span>
								</label>
							</div>
						...
						{uploadedItems.map((item) => (
							...
							>
								<h3 className="font-medium">
									{item.sections?.traditionalpageone?.["dc.title"]?.[0]
										?.value || "Untitled"}
								</h3>
